...
function mockDecision(sourcePacket = {}) {
  ...
  if (firstCandidate) {
    return normalizeHiveDecisionOutput({
      explanation: "The reports and live task state show at least one idle badge-eligible contributor. In shadow mode, the Decision Agent would consider routing a scoped task only after structural dedup confirms the contributor does not already have matching outstanding or rewarded work.",
      options_considered: [
        {
          action: "create_task",
      ...
      payload: {
        ...
        required_badge_id: firstCandidate.defaultBadge || firstCandidate.verifiedBadges?.[0] || "",
        task_work_type: firstCandidate.allowedWorkTypes?.[0] || "shadow_review",
        ...
        routing_reason: "Idle eligible contributor present; testing non-stall routing guardrail in shadow mode.",
