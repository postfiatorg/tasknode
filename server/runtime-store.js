...
export function createTerminalAuthRequest({
  provider = "github",
  origin = "",
  userAgent = "",
  ip = "",
...
export function getTerminalAuthRequest({ requestId = "" } = {}) {
...
export function completeTerminalAuthRequest({
  requestId = "",
  accountId = "",
  provider = "github",
} = {}) {
...
function createTerminalSession({ accountId = "", provider = "github" } = {}) {
  ...
  if (!normalizedAccountId || !linkedProvider) {
    return { ok: false, error: "github_not_linked" };
  }
  ...
  return {
    ok: true,
    token,
    session: terminalSessionPayload(state.terminalSessions[sessionId]),
  };
}
...
export function consumeTerminalAuthRequestSession({
  requestId = "",
  pollToken = "",
} = {}) {
  pruneExpiredTerminalAuthRequests({ save: false });
  ...
  const issued = createTerminalSession({
  ...
  return {
    ok: true,
    status: 200,
    terminalToken: issued.token,
    session: issued.session,
  };
}

export function getTerminalSessionByToken(token = "") {
  ...
  if (!session) return null;
  if (!accountHasLinkedProvider({ accountId: session.accountId, provider: session.provider })) return null;
  return terminalSessionPayload(session);
}

export function revokeTerminalSessionByToken(token = "") {
  const hash = tokenHash(token);
  if (!hash) return false;
  let revoked = false;
  for (const [sessionId, session] of Object.entries(state.terminalSessions || {})) {
    if (!session?.tokenHash) continue;
    if (!timingSafeEqual(Buffer.from(session.tokenHash), Buffer.from(hash))) continue;
    delete state.terminalSessions[sessionId];
    revoked = true;
  }
