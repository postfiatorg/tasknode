...
import { getLatestTaskAccountingHarvestReport } from "./repositories/task-accounting-harvester.js";
import {
...
const maxHiveReportsContextCharacters = 36_000;
const maxHiveReportBodyCharacters = 3_400;
const maxHiveHarvestReportCharacters = 4_800;
const maxHiveLiveTaskPacketCharacters = 7_500;
const hiveImmediatePromptPath = "hive/hive_immediate_response_v1.md";
const hiveNetworkTaskRoutingPolicyPromptPath = "hive/hive_network_task_routing_policy_v1.md";
const hiveImmediatePrompt = loadPrompt(hiveImmediatePromptPath);
const hiveNetworkTaskRoutingPolicyPrompt = loadPrompt(hiveNetworkTaskRoutingPolicyPromptPath);
...
function textWithTruncation(value = "", max = 1000, marker = "[truncated]") {
  const text = String(value || "").trim();
  if (text.length <= max) return text;
  return `${text.slice(0, max).trimEnd()}\n${marker}`;
}

function formatReportForImmediateResponse(report = null, type = "") {
  const normalizedType = safeText(type || report?.type, 80);
  const label = hiveReportTypes[normalizedType]?.label || safeText(report?.label || normalizedType, 120);
  if (!report?.id) {
    return [
      `## ${label || normalizedType || "Unknown"} Report`,
      `Report key: ${normalizedType || "unknown"}`,
      "Status: not generated or unavailable.",
    ].join("\n");
  }
  const body = textWithTruncation(
    report.bodyMarkdown || report.bodyExcerpt || "",
    maxHiveReportBodyCharacters,
    "[report body clipped for Hive Chat prompt budget]"
  );
  return [
    `## ${label} Report`,
    `Report key: ${normalizedType}`,
    `Report id: ${safeText(report.id, 180)}`,
    report.generatedAt ? `Generated: ${report.generatedAt}` : "",
    report.model ? `Model: ${safeText(report.model, 180)}` : "",
    "",
    body || "Report body unavailable.",
  ].filter(Boolean).join("\n");
}

function formatHarvestReportForImmediateResponse(result = null) {
  const report = safeObject(result).report || null;
  const summary = safeObject(result?.summary);
  const summaryLine = Object.keys(summary).length
    ? [
        `resolved=${Number(summary.resolved || 0)}`,
        `unresolved=${Number(summary.unresolved || 0)}`,
        `requires_action=${Number(summary.requires_action || summary.requiresAction || 0)}`,
        `checked_out=${Number(summary.checked_out || summary.checkedOut || 0)}`,
      ].join(" | ")
    : "";
  if (!report?.id) {
    return [
      "## Harvest Report",
      "Status: not generated or unavailable.",
      summaryLine ? `Backlog summary: ${summaryLine}` : "",
    ].filter(Boolean).join("\n");
  }
  return [
    "## Harvest Report",
    `Report id: ${safeText(report.id, 180)}`,
    report.generatedAt ? `Generated: ${safeText(report.generatedAt, 80)}` : "",
    `Resolved harvests: ${Number(report.resolvedCount || 0)}`,
    `Unresolved harvests: ${Number(report.unresolvedCount || 0)}`,
    "",
    textWithTruncation(
      report.bodyMarkdown || "",
      maxHiveHarvestReportCharacters,
      "[harvest report clipped for Hive Chat prompt budget]"
    ) || "Harvest report body unavailable.",
  ].filter(Boolean).join("\n");
}

function formatLiveTaskPacketForImmediateResponse(result = null) {
  const packet = safeObject(result).packet || null;
  if (!packet?.text) {
    return [
      "## Live Task Packet",
      "Status: not generated or unavailable.",
      result?.message ? `Message: ${safeText(result.message, 300)}` : "",
    ].filter(Boolean).join("\n");
  }
  return [
    "## Live Task Packet",
    packet.generatedAt ? `Generated: ${safeText(packet.generatedAt, 80)}` : "",
    `Contributors in packet: ${Number(packet.contributorCount || 0)}`,
    "",
    textWithTruncation(
      packet.text,
      maxHiveLiveTaskPacketCharacters,
      "[live task packet clipped for Hive Chat prompt budget]"
    ),
  ].filter(Boolean).join("\n");
}

export function formatHiveReportsContextForImmediateResponse({
  reports = [],
  harvestReport = null,
  liveTaskPacket = null,
  generatedAt = new Date().toISOString(),
} = {}) {
  const reportsByType = new Map(safeArray(reports).map((report) => [safeText(report?.type, 80), report]));
  const reportBlocks = hiveReportTypeIds.map((type) => formatReportForImmediateResponse(reportsByType.get(type), type));
  return safeText(
    [
      "HIVE REPORTS CONTEXT - AUTHORITATIVE STRATEGIC BACKGROUND",
      "This is a plain-text packet of generated Hive reports and deterministic task/harvest packets. Use it to understand what the Hive currently believes, what is unresolved, what operators are doing, and what clarifying information would improve the network. Prefer Account Live State and Live Board Facts for current user-specific task status.",
      `Packet generated: ${safeText(generatedAt, 80)}`,
      "",
      ...reportBlocks,
      "",
      formatHarvestReportForImmediateResponse(harvestReport),
      "",
      formatLiveTaskPacketForImmediateResponse(liveTaskPacket),
    ].filter(Boolean).join("\n\n"),
    maxHiveReportsContextCharacters
  );
}

async function buildHiveReportsContextForImmediateResponse({ now = new Date() } = {}) {
  const [reports, harvestReport, liveTaskPacket] = await Promise.all([
    Promise.all(hiveReportTypeIds.map((type) => latestHiveReport({ type }).catch(() => null))),
    getLatestTaskAccountingHarvestReport({ generate: false }).catch((error) => ({
      ok: false,
      error: safeText(error?.message || String(error), 300),
      report: null,
    })),
  ...
  const availableReports = reports.filter(Boolean);
  const text = formatHiveReportsContextForImmediateResponse({
  ...
  return {
    ...
    reportIds: availableReports.map((report) => safeText(report?.id, 180)).filter(Boolean),
    reportTypes: hiveReportTypeIds,
    harvestReportId: safeText(harvestReport?.report?.id, 180),
    liveTaskPacketGeneratedAt: safeText(liveTaskPacket?.packet?.generatedAt, 80),
  };
}
...
function hiveSystemPrompt({
  ...
  const reportsText = safeText(hiveReportsContextText, maxHiveReportsContextCharacters);
  const userText = safeText(requestingUserText, 4000);
  return renderPromptTemplate(hiveImmediatePrompt, {
    requesting_user_section: userText || "",
    account_live_state_section: accountStateText || "",
    network_task_routing_policy_section: networkTaskRoutingPolicyForPrompt(),
    requesting_user_hive_context_source_packet_section: packetText
      ? ["REQUESTING USER HIVE CONTEXT SOURCE PACKET", "This packet is scoped to the requesting account, not the entire Hive.", packetText].join("\n")
      : "",
    hive_mind_context_section: boardText || "",
    hive_reports_context_section: reportsText || "",
  });
}
...
export async function executeHiveImmediateResponse({
  ...
  const [historyMessages, sourcePacket, accountLiveState, hiveMindContext, hiveReportsContext] = await Promise.all([
    ...
    buildHiveReportsContextForImmediateResponse().catch(() => ({
      text: "",
      digest: "",
      reportIds: [],
      reportTypes: hiveReportTypeIds,
      harvestReportId: "",
      liveTaskPacketGeneratedAt: "",
    })),
  ]);
    ...
    messages: [
      {
        ...
        content: hiveSystemPrompt({
          ...
          hiveReportsContextText: hiveReportsContext.text,
          requestingUserText,
        }),
      },
  ...
  try {
    ...
    return {
      ...
      hiveReportsContextDigest: hiveReportsContext.digest,
      hiveReportsContextReportIds: hiveReportsContext.reportIds,
      hiveReportsContextReportTypes: hiveReportsContext.reportTypes,
      hiveHarvestReportId: hiveReportsContext.harvestReportId,
      hiveLiveTaskPacketGeneratedAt: hiveReportsContext.liveTaskPacketGeneratedAt,
      promptPath: hiveImmediatePromptPath,
      promptDigest: promptDigest(hiveImmediatePrompt),
    };
  } catch (error) {
