...
function compactCapacityCheckBlocker(blocker = {}, check = {}) {
  return {
    ...
    rewardOfferPft: numeric(blocker.rewardOfferPft || blocker.reward_offer_pft, 0),
...
function activeNetworkTaskCapacityBlockers({
  ...
  return blockerRows.map(({ task, source }) => ({
    ...
    rewardOfferPft: numeric(task.rewardOfferPft || task.reward_offer_pft || task.rewardPft || task.reward_pft, 0),
    acceptBy: safeText(task.acceptBy || task.accept_by, 80),
    deadlineAt: safeText(task.deadlineAt || task.deadline_at, 80),
