...
import {
  callRecommendedConnectionsProvider as callDeepSeekRecommendedConnections,
  deterministicRecommendedConnections,
  maxRecommendedConnections as maxRecommendations,
  minRecommendedConnections as minRecommendations,
  publicRecommendedConnection as publicConnection,
  publicRecommendedConnectionCandidate as publicCandidate,
  recommendedConnectionCandidateSnapshot as candidateSnapshot,
  recommendedConnectionRunIsFresh as latestRunFresh,
} from "./recommended-connections-ranking.js";
...
async function listRecommendedConnectionCandidateAccountIds({ limit = 20 } = {}) {
  ...
  const visibility = await Promise.all(accountIds.map((accountId) => getAccountProfileVisibility({ accountId })));
  return accountIds.filter((_accountId, index) => shouldIndexRecommendedConnectionProfile(visibility[index]));
}
...
export async function buildRecommendedConnectionPacket({ accountId = "" } = {}) {
  ...
  const visibility = await getAccountProfileVisibility({ accountId: normalizedAccountId });
  ...
  const walletIdentity = (await listDiscoverableAccountWalletIdentities())
