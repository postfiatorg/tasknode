...
export async function getNetworkTaskEligibility({
  ...
  const base = {
    ...
    policy: {
      ...
      requiresActivePftlWalletSync: false,
      requiresCompletedNetworkDiagnosticReport: false,
  ...
  const candidateVerdict = await explainNetworkTaskCandidateEligibility({ accountId: normalizedAccountId, explicitWallet: normalizedWalletAddress });
  const capacityState = await getNetworkTaskCapacityState({ accountId: normalizedAccountId, walletAddress: candidateVerdict.walletAddress || normalizedWalletAddress });
  const capacityAvailable = capacityState.available;
  ...
  const gates = [
    ...
    eligibilityGate(
      ...
      walletSynced ? "complete" : "waiting",
      walletSynced
        ? "The linked wallet is active in the PFTL sync cache."
        : normalizedWalletAddress
          ? "Wallet indexing enriches history; it does not block otherwise eligible Network Task routing."
    ...
    eligibilityGate(
      ...
      profile
        ...
        : ["pending", "processing"].includes(profileStatus)
          ...
          : "The report enriches routing context; its generation does not block an otherwise eligible account.",
    ...
    eligibilityGate(
      ...
      !badgeEligible || !capacityAvailable ? "blocked" : "complete",
      !badgeEligible
    ...
    eligibilityGate(
      ...
      candidateVerdict.eligible && capacityAvailable ? "waiting" : "blocked",
  ...
  const eligibility = {
    ...
    summary: ready
      ...
      : "Network Task routing needs a delivery wallet, a verified operating badge, and free task capacity. Reports and wallet indexing enrich the context in the background.",
    ...
    capacity: {
      ...capacityState,
      available: candidateVerdict.eligible && capacityAvailable,
      blockers,
