...
export async function migrateDatabase({ force = false } = {}) {
  ...
  for (const name of await discoverMigrationNames()) {
    ...
    const appliedByThisProcess = await transaction(async (client) => {
      // Every Fly process starts from the same image and may reach migration
      // startup concurrently. Serialize each migration and re-check its marker
      // inside the lock so workers never execute the same DDL/data repair in
      // parallel with the web process.
      await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [`tasknode:migration:${name}`]);
      const current = await client.query(`SELECT 1 FROM ${migrationsTable} WHERE name = $1`, [name]);
    ...
    if (appliedByThisProcess) appliedNow.push(name);
  }
