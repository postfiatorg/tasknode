...
Task Node is licensed under the permissive MIT License. This repository is the
canonical source for the application running at `tasknode.postfiat.org`,
including its production workers, deployment topology, migrations, and
operator tooling. There is no private application-code superset.

Production credentials and user data are supplied by the deployment platform
and are never committed. Every production image must identify the public Git
commit from which it was built.
...
ops/                         Production service definitions and operator tooling
...
`src/features/docs/docs-content.js`; its public-source boundary is enforced by
