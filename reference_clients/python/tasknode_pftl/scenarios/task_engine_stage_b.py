...
FIXTURES_DIR = Path(__file__).resolve().parents[2] / "tasknode_pftl" / "fixtures"
STAGE_B_CASES_PATH = FIXTURES_DIR / "stage_b_cases.json"
...
def stage_b_case_for_index(index: int) -> StageBCase:
    cases = load_stage_b_cases(exclude_replacements=True)
    return cases[index % len(cases)]


def load_stage_b_cases(*, exclude_replacements: bool = False) -> list[StageBCase]:
    raw_cases = json.loads(STAGE_B_CASES_PATH.read_text(encoding="utf-8"))
    cases = [StageBCase(**item) for item in raw_cases]
    if exclude_replacements:
        cases = [case for case in cases if not case.label.startswith("rerequest-")]
    if not cases:
        raise RuntimeError("stage_b_cases_fixture_empty")
    return cases


def stage_b_case_by_label(label: str) -> StageBCase:
    for case in load_stage_b_cases():
        if case.label == label:
            return case
    raise RuntimeError(f"stage_b_case_missing:{label}")
...
def run_stage_b_user(
    ...
    **kwargs: Any,
    ...
    if case.re_request_after_refusal:
        replacement_case = stage_b_case_by_label("rerequest-replacement")
