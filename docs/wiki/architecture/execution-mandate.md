...
Review implementation against this document (execution mandate). Mark each item when verified.
...
- [ ] Verification commands in doc still exist in `package.json`.
- [ ] Status language definitions used consistently in PRs and docs updates.
...
- [ ] Mandate stays procedural; feature detail remains in surface/architecture pages.
...
- [ ] Claims of 'fixed' require live verification, not unit-test-only proof for UX paths.
