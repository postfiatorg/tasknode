...
Review implementation against this document (resettable-signup-testing). Mark each item when verified.

### Memory Efficiency
- [ ] Operator reset handles bounded account-scoped rows and does not scan unrelated large payloads beyond runtime-store maps.

### Code Quality
- [ ] Commands, env vars, and file paths verified against repo and Docker layout.
- [ ] Reset command dry-runs by default and writes an audit event on mutation.
- [ ] Faucet reset requires an explicit `--grant-mode reset`.

### Coherence
- [ ] Workflow aligns with Auth, Wallet, Database, and Ethereum top-up docs.
- [ ] Document distinguishes identity reset from value custody and data erasure.

### Bloat
- [ ] Document stays process-focused and does not duplicate full auth or wallet specs.

### Security
- [ ] No secrets, codes, private keys, or mnemonics are included.
- [ ] Custody boundaries are explicit for faucet grants and Ethereum deposits.
