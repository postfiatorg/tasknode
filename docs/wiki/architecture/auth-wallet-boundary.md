...
Review implementation against this document (auth wallet boundary). Mark each item when verified.
...
- [ ] Regression tests cited still pass (`route-smoke`, wallet smokes).
- [ ] Error codes documented match server responses.

### Coherence
- [ ] Doc aligns with wiki and spec docs for same topic.
- [ ] Three boundaries (account auth, wallet proof, vault unlock) consistently applied in UI and API.
- [ ] Initiation grant rules match wallet surface doc.

### Bloat
- [ ] Engineering doc scoped to its audience; defers product detail to wiki.
- [ ] Guardrail doc avoids repeating full OAuth spec from auth_account_spec.

### Security
- [ ] No secrets committed; custody boundaries explicit.
- [ ] Anti-patterns section checked against current `main.jsx` and auth routes.
- [ ] Seed never transmitted; wallet proof challenge freshness enforced.
