...
| Hive Secretary | OpenAI Responses | `gpt-5.5-pro`; `high` | Deprecated historical/context primitive; no upgrade planned. If revived, map to `gpt-5.6-sol` with Responses `reasoning.mode: "pro"` and explicit `reasoning.effort: "high"`. | `RECOMMEND-ONLY` | The current Project Status surface is not written by GPT 5.5 Pro jobs; revival changes request shape and autonomous-worker cost behavior. `docs/wiki/surfaces/hive.md:714,829` | unknown/not comparable | `server/hive-secretary-worker.js:44-50` |
| Hive Project | OpenAI Responses | `gpt-5.5-pro`; `high` | Deprecated historical/context primitive; no upgrade planned. If revived, map to `gpt-5.6-sol` with Responses `reasoning.mode: "pro"` and explicit `reasoning.effort: "high"`. | `RECOMMEND-ONLY` | The current Project Status surface is not written by GPT 5.5 Pro jobs; revival changes request shape and autonomous-worker cost behavior. `docs/wiki/surfaces/hive.md:714,829` | unknown/not comparable | `server/hive-project-worker.js:26-39` |
| Board Manager decision | OpenRouter by default; legacy OpenAI gated | `z-ai/glm-5.2`; `high` (legacy fallback `gpt-5.5-pro`) | Evaluate separately | `RECOMMEND-ONLY` | GLM 5.2 and legacy-provider gating require explicit policy. | unknown/not comparable | `server/board-manager-decision-provider.js:31-49` |
...
| Hive decision agent | OpenRouter Chat Completions | `z-ai/glm-5.2`; `high` | Evaluate separately | `RECOMMEND-ONLY` | GLM 5.2 and decision-agent semantics. | unknown/not comparable | `server/hive-decision-agent-provider.js:25-42` |
...
| Board Manager loop/model executor/worker | OpenRouter `z-ai/glm-5.2`; `high`; legacy OpenAI `gpt-5.5-pro` | Board Manager is opt-in and GLM/legacy policy remains — `RECOMMEND-ONLY` | `scripts/board-manager-loop.mjs:36-37,52-54,140,148`; `scripts/board-manager-model-exec.mjs:70,126`; `scripts/board-manager-worker.mjs:51,71-72`; `package.json:79,137-143` |
...
| `gpt-5.5-pro` | Focused smoke expectation — `FIXTURE-ONLY` | `scripts/board-manager-glm52-cost-smoke.mjs:21`; `scripts/board-manager-v0-smoke.mjs:564,585`; `scripts/hive-project-planning-smoke.mjs:76` |
| `gpt-5.5-pro-2026-04-23` | Focused dated-model expectation — `FIXTURE-ONLY` | `scripts/board-manager-v0-smoke.mjs:572,594`; `scripts/hive-project-planning-smoke.mjs:40,83` |
...
| --- | --- | --- |
| `gpt-5.5-pro` | Hive UI historical/provider display — `NO-CHANGE` | `src/features/hive/HiveBrainView.jsx:230,242` |
...
| `z-ai/glm-5.2` production/default/deploy/display | All GLM changes remain `RECOMMEND-ONLY` | `server/board-manager-decision-provider.js:38,41,43`; `server/chat-router.js:70`; `server/context-rewrite-provider.js:129,131,136`; `server/expert-badge.js:41`; `server/hive-board-secretary-provider.js:25`; `server/hive-decision-agent-provider.js:30`; `server/hive-report-provider.js:6`; `server/hive-task-manager-provider.js:29`; `server/system-status.js:35,815`; `docker-compose.dev.yml:176`; `fly.toml:77,85` |
...
| `gpt-5.5-pro` active default/display | Current active/default references map `RECOMMEND-ONLY` to `gpt-5.6-sol` with Responses `reasoning.mode: "pro"` and explicit reasoning effort; implementation requires request-shape/cost approval. Legacy/display references are not change targets. | `server/board-manager-decision-provider.js:43`; `server/hive-project-worker.js:27`; `server/hive-secretary-worker.js:45`; `server/system-status.js:39` |
