...
| `z-ai/glm-5.2` production/default/deploy/display | Active GLM routes remain catalogued; Secretary/Project now have explicit Fly pins, while unrelated GLM routes remain `RECOMMEND-ONLY` | `server/board-manager-decision-provider.js:38,41,43`; `server/chat-router.js:70`; `server/context-rewrite-provider.js:129,131,136`; `server/expert-badge.js:41`; `server/hive-board-secretary-provider.js:25`; `server/hive-decision-agent-provider.js:30`; `server/hive-report-provider.js:6`; `server/hive-task-manager-provider.js:29`; `server/system-status.js:35,815`; `docker-compose.dev.yml:176`; `fly.toml` |
