...
| `network_projects` | Active Hive project records that exist before task allocation. Stores type, title, summary, objective/about text, phase, priority, routed PFT snapshot, task/contributor counts, and latest Hive Secretary input reference. | Hive active project cards, Hive project detail, future network task allocation. | `029_hive_network_projects.sql` |
| `network_project_contributors` | Project-scoped contributor/operator rollups with wallet, codename, archetype, load/capacity, earned PFT snapshot, and display ordering. | Hive contributor previews, allotted operators, project detail contributors. | `029_hive_network_projects.sql` |
| `network_project_task_refs` | Project-scoped task rows. `task_id` is empty until a concrete PFTL task is allocated; the row can still hold an apriori project work item. | Hive project task board, future project-to-PFTL task linking. | `029_hive_network_projects.sql` |
| `network_project_activity` | Project-scoped activity feed rows with wallet, action, task label, routed PFT amount, and display ordering. | Hive routing feed and project activity section. | `029_hive_network_projects.sql` |
