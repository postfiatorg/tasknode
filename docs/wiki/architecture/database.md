...
| `hive_secretary_reports` | Completed Hive Secretary reports over validated-wallet Hive chat entries, with source packet, structured provider output JSON/text, provider/model/prompt metadata, usage JSON, and supersession timestamp. Current generation uses OpenRouter GLM 5.2; historical provider/model values remain readable. | Hive page Secretary report, Hive Active Projects worker input, future system Network Task worker input, prompt audit. | `028_hive_secretary_reports.sql` |
| `hive_project_planning_jobs` | Async queue rows for turning the latest Hive Secretary report into active project records with the OpenRouter GLM 5.2 planning worker. | Hive Active Projects worker, Hive project diagnostics. | `031_hive_project_planning.sql` |
| `hive_project_generations` | Completed active-project generations with source report, structured provider output, provider/model/prompt metadata, response id, and usage JSON. Current generations use OpenRouter GLM 5.2; historical provider/model values remain readable. | Hive active project audit, project registry rebuilds, future network task allocation. | `031_hive_project_planning.sql` |
