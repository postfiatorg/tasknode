...
Postgres is durable product storage for accounts, chat, billing, Context,
Memory, current offchain task events, projections and work queues. Historical
signed PFTL/IPFS data also has a cache/reducer path. Current Postgres-native
tasks require database backup and recovery; wallet-history replay alone cannot
restore them. Reviewed September 5, 2026; this table inventory includes
historical components and is not a declaration that every worker is enabled.
...
- Password credentials and retained browser account sets: `server/db/migrations/128_multi_account_password_wallet_isolation.sql`
- Team Context reports, preferences, and jobs: `server/db/migrations/130_team_context_reports.sql`
...
| `legacy_pftasks_chat_messages` | Read-only PFTasks message archive keyed by immutable source message id and wallet. Conversation ids are deterministic from source user plus chat type. | Wallet-authorized recovery of pre-cutover chat recents, history, and Search. | `127_legacy_pftasks_history.sql` |
...
| `chat_model_runs` | One row per model execution with provider, model, mode, response IDs, status, token counts, prompt-cache hit/miss accounting, cache reporting state and savings, cost source, web search calls, tool cost, model cost, and total cost. | Chat usage display, cache-efficiency audit, billing ledger creation, cost audit, provider reliability review. | `001_chat_billing.sql`, `107_chat_prompt_cache_accounting.sql` |
...
| `billing_ledger_entries` | Append-style billing ledger for credits and debits, including model run linkage, token and prompt-cache usage, cache savings and cost source, provider/model, source, idempotency key, and metadata. | Wallet/Billing page, usage and cache audit, top-up accounting, chat cost history. | `001_chat_billing.sql`, `107_chat_prompt_cache_accounting.sql` |
...
| `account_password_credentials` | One optional Argon2id credential per account, including credential version, use timestamps, and soft-disable state. Password mutations revoke existing account sessions. | Password login, password setup/change/disable, email-based password reset. | `128_multi_account_password_wallet_isolation.sql` |
| `device_account_sets` | Opaque, expiring, revocable browser account-set records. Only a hash of the browser token is stored. | Profile account switcher, add-account flow, current-account logout, logout-all. | `128_multi_account_password_wallet_isolation.sql` |
| `device_account_set_members` | Accounts explicitly authenticated into a retained browser account set, with authentication and selection timestamps plus revocation state. | Account listing, switching, removal, and session-set revocation. | `128_multi_account_password_wallet_isolation.sql` |
...
| `legacy_pftasks_context_revisions` | Read-only legacy context revision CIDs keyed by source revision id and wallet. Duplicate CIDs are suppressed when merged into Context history. | Recovery of pre-cutover context versions, including revisions without a projected PFTL pointer. | `127_legacy_pftasks_history.sql` |
...
| `team_context_preferences` | Account-scoped opt-in controlling whether a current authorized Team Context report is composed into personal model context. | Team checkmark, chat and wallet-agent execution context. | `130_team_context_reports.sql` |
| `team_context_reports` | Latest generated one-page report per viewer account, keyed by the exact grants, identities, and rewarded-task source fingerprint with provider/model/usage audit metadata. | Team Context report and opted-in personal context. | `130_team_context_reports.sql` |
| `team_context_jobs` | Digest-aware async generation queue with claim, retry, lock, and error state. Reward changes fan out jobs only to active task-history viewers. | Team Context worker and Team-page update state. | `130_team_context_reports.sql` |
...
| `hive_secretary_reports` | Completed Hive Secretary reports over validated-wallet Hive chat entries, with source packet, structured provider output JSON/text, provider/model/prompt metadata, usage JSON, and supersession timestamp. Current generation uses Vercel GLM 5.3 with Ambient backup; historical provider/model values remain readable. | Hive page Secretary report, Hive Active Projects worker input, future system Network Task worker input, prompt audit. | `028_hive_secretary_reports.sql` |
| `hive_project_planning_jobs` | Async queue rows for turning the latest Hive Secretary report into active project records with the Vercel-first GLM 5.3 planning worker. | Hive Active Projects worker, Hive project diagnostics. | `031_hive_project_planning.sql` |
| `hive_project_generations` | Completed active-project generations with source report, structured provider output, provider/model/prompt metadata, response id, and usage JSON. Current generations use Vercel GLM 5.3 with Ambient backup; historical provider/model values remain readable. | Hive active project audit, project registry rebuilds, future network task allocation. | `031_hive_project_planning.sql` |
...
| `board_manager_secretary_packets` | Reusable Board Manager Secretary packets keyed by semantic source digest. Stores compact packet JSON/text, provider/model/prompt metadata, response id, usage, status, and supersession timestamps. | Retained Board Manager decision source, Hive Mind Agent audit follow-up, provider cost diagnostics. | `043_board_manager_secretary_packets.sql` |
...
| `legacy_pftasks_tasks` | Read-only pre-cutover task archive keyed by immutable source task id and wallet. These rows never enter active projections, routing, rewards, or worker capacity. | Historical Task cards and detail evidence for users who link the source wallet. | `127_legacy_pftasks_history.sql` |
| `legacy_pftasks_import_runs` | Audited counts for each idempotent bulk or wallet-scoped archive import. | Migration verification and operator recovery evidence. | `127_legacy_pftasks_history.sql` |
| `task_accounting_harvests` | Post-reward accounting queue for rewarded Network Tasks. Stores the task/reward source packet, DeepSeek classification, requires-action flag, suggested action, provider/model metadata, worker state, resolution state, and current checkout assignment. | Hive Brain Harvests tab, operator accounting follow-up queue. | `083_task_accounting_harvester.sql`, `084_task_accounting_harvest_resolution.sql`, `085_task_accounting_harvest_checkout.sql` |
| `task_accounting_harvest_checkout_events` | Append-only log of Core Contributor checkout actions for Task Accounting harvest rows, including account, wallet, metadata, and timestamp. | Hive Brain checked-out log and accounting ownership audit. | `085_task_accounting_harvest_checkout.sql` |
