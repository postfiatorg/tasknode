...
| `task_accounting_harvests` | Post-reward accounting queue for rewarded Network Tasks. Stores the task/reward source packet, DeepSeek classification, requires-action flag, suggested action, provider/model metadata, and worker state. | Hive Brain Harvests tab, operator accounting follow-up queue. | `083_task_accounting_harvester.sql` |
