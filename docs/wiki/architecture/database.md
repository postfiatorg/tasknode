...
| `board_manager_user_messages` | User-visible Board Manager messages keyed by account and run id. These are generated only by the `message_user` action hook and are separate from raw Hive Input. | Hive Mind Agent message block, future user notification surfaces. | `035_board_manager_action_hooks.sql` |
