...
Status: deprecated implemented v1 plan. Current product truth lives in `Surfaces -> Memory`, `Surfaces -> Hive`, and `Architecture -> Network Task Profile Worker`.
...
Review implementation against this document (network task profile memory plan). Mark each item when verified.
