...
| --- | --- | --- | --- |
| Jobs XML prompt | `docs/jobs.md` | 29,333 bytes | Candidate Phase 1 system prompt. It contains XML prompt text with runtime slots for context, current plate, retrieved Jobs essence, and current user message. |
...
Phase 1 should not require pgvector or the full gist corpus. Phase 1 only needs the Jobs XML prompt wired into the shared chat instruction path and tested across the four chat modes.
...
1. Promote the local XML prompt artifact into a versioned runtime prompt file, likely `prompts/chat/jobs_chat_os_v1.md`.
...
jobs_chat_os_v1.md rendered with:
  ...
  CURRENT_PLATE = formatted task context plus formatted memory context
...
| File | Purpose |
| --- | --- |
| `prompts/chat/jobs_chat_os_v1.md` | Versioned Jobs XML prompt. |
...
- Whether the runtime prompt file should be named exactly `Jobs.md` for continuity or normalized to `prompts/chat/jobs_chat_os_v1.md` for versioning.
