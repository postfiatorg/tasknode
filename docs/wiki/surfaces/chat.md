...
- Provider failure should show an explicit error turn. Terminal provider errors are not retried as network failures.
- A browser network failure or non-terminal stream close retries automatically, clears partial assistant output before reconnecting, and restores the original composer draft if all reconnect attempts fail.
