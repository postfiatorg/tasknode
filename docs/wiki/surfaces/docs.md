...
6. Use the document chat for human discussion. `@ODV` and `@coach` are explicit Ambient GLM 5.2 actions.
...
CryptPad links are bearer capabilities. Revoking a Task Node grant removes normal library delivery but cannot erase a capability that a recipient already copied. Rotate the PFDocs capability/password and re-share when an edit link may be compromised.
