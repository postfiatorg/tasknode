...
Network Task Profile jobs use the same Ambient DeepSeek Flash memory worker. The prompt is `prompts/memory/network_task_profile_v2.md`. The API route is `GET /api/memory/network-task-profile`; `POST /api/memory/network-task-profile` requests an explicit rebuild from the Memory page. Generation is automatic: the memory worker sweeps active linked-wallet accounts that lack a completed profile, and eligibility reads self-heal a missing profile by queueing it server-side. Recent failures are retried after a cooldown. Opening Memory is not required. Rewarded task projections can still trigger refreshes when routing context changes. There is no request, application, or approval flow for the report, and chat surfaces must not tell users to ask for one; the Hive and Help prompts state this same path. The generated profile is not required for the page to render. Network Context Inputs are built from profile data and routable `task_projections` on every route read and are returned even while a profile job is pending.
