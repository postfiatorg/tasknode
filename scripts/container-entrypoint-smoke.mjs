...
assert.equal(
  (dockerfile.match(/COPY --chown=node:node --from=build \/runtime-(?:web|worker) \.\//g) || []).length,
  4,
  "every runtime source tree must be owned by the unprivileged node process"
);
assert.equal(
  (dockerfile.match(/COPY --chown=node:node --from=build \/app\/dist \.\/dist/g) || []).length,
  2,
  "every frontend runtime tree must be owned by the unprivileged node process"
);
