...
async function command(t, argv, status, { outsideScope = false, databaseFailure = false, priorRejections = 0 } = {}) {
  const statements = [];
  const rejections = [];
  const query = async (input, params = []) => {
    const sql = typeof input === "string" ? input : input.text;
    statements.push(sql);
    if (sql.includes("FROM bm_audit_log") && sql.includes("lifecycle_rejected")) return { rows: [{ count: priorRejections }] };
    if (sql.includes("INSERT INTO bm_audit_log")) { rejections.push(params); return { rows: [{ id: "bmaudit_fixture", created_at: "2026-09-20T00:00:00.000Z" }] }; }
  ...
  return { ...response, errors, statements, rejections };
}
...
for (const status of ["submitted", "accepted", "verification_requested", "rewarded"]) {
  test("review from " + status + " returns an actionable conflict, not a retryable failure", async (t) => {
    ...
    // Structured fields let the CLI and supervisor act on the conflict.
    assert.deepEqual({ taskId: result.body.lifecycle.taskId, taskStatus: result.body.lifecycle.taskStatus, command: result.body.lifecycle.command, boardId: result.body.lifecycle.boardId },
      { taskId, taskStatus: status, command: "review", boardId });
    assert.equal(result.body.lifecycle.priorRejections, 0);
    // The rejection is recorded durably, after the command transaction rolled back.
    assert.equal(result.rejections.length, 1);
    assert.ok(result.statements.indexOf("ROLLBACK") < result.statements.findIndex((sql) => sql.includes("INSERT INTO bm_audit_log")));
    assert.equal(result.rejections[0][3], "lifecycle_rejected");
    assert.deepEqual(JSON.parse(result.rejections[0][4]), { taskId, taskStatus: status, command: "review", argv: ["review", taskId, "--decision", "reward", "--pft", "100"] });
  });
}
test("a repeated invalid review names its prior rejections and says repeating cannot succeed", async (t) => {
  const result = await command(t, ["review", taskId, "--decision", "reward", "--pft", "100"], "submitted", { priorRejections: 16 });
  assert.equal(result.code, 409);
  assert.equal(result.body.lifecycle.priorRejections, 16);
  assert.ok(result.body.message.includes("already been rejected 16 time(s)"));
  assert.ok(result.body.message.includes("cannot succeed until the task status changes"));
  assert.ok(result.body.message.includes("bm verify request " + taskId));
  assert.deepEqual(JSON.parse(result.rejections[0][5]).rejection, 17);
});
for (const status of ["accepted", "verification_response_submitted", "rewarded"]) {
  test("verify request from " + status + " returns an actionable conflict", async (t) => {
    ...
    if (status === "verification_response_submitted") assert.ok(result.body.message.includes("bm review " + taskId));
    assert.equal(result.body.lifecycle.command, "verify request");
    assert.equal(result.rejections.length, 1);
    assert.equal(result.errors.mock.callCount(), 0);
  });
}
test("scope denial stays forbidden before lifecycle information is exposed", async (t) => {
  ...
  assert.equal(result.body.lifecycle, undefined);
