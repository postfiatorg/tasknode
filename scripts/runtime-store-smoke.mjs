...
try {
  ...
  if (topUpAction?.enabled !== true) throw new Error(`Ethereum top-up action should be enabled with an xpub: ${JSON.stringify(topUpAction)}`);

  const noLoginTopUp = await usageTopUpStart({}, "POST", null);
  if (noLoginTopUp.status !== 401 || noLoginTopUp.body?.error !== "usage_top_up_login_required") throw new Error(`Ethereum top-up should require account login: ${JSON.stringify(noLoginTopUp)}`);
  ...
  global.fetch = async (url, options = {}) => {
    ...
    if (payload.method === "eth_getBalance") return new Response(JSON.stringify({ jsonrpc: "2.0", id: payload.id, result: "0x0" }), { status: 200 });
