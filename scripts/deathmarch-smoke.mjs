...
const providerUnavailableFallback = await callDeepSeekSummary({
    event: sensitiveEvent,
    anonymity: 3,
    classification: { level: 1, category: "classification unavailable" },
    env: {
...
assert.equal(providerUnavailableFallback.includes("**Task requested**"), true);
assert.equal(providerUnavailableFallback.includes("autocorrelation"), false);
assert.equal(providerUnavailableFallback.includes("ACME"), false);
...
await assert.rejects(
  () => callDeepSeekSummary({
    ...
    env: {
      AMBIENT_API_KEY: "test",
      AMBIENT_BASE_URL: "https://ambient.invalid",
      DEATHMARCH_DETERMINISTIC_FALLBACK: "false",
    },
    fetchImpl: async () => ({
      ok: false,
      status: 503,
...
assert.equal(failedClassifierPosts[0].content.includes("ACME"), false);
...
assert.equal(continuedAfterEventFailure.ok, true);
assert.equal(continuedAfterEventFailure.failed, 0);
assert.equal(continuedAfterEventFailure.posted, 2);
assert.equal(continuedPosts.length, 2);
assert.equal(continuedPosts[0].content.includes("**Task requested**"), true);
assert.equal(continuedPosts[0].content.includes("autocorrelation"), false);
assert.equal(continuedPosts[0].content.includes("ACME"), false);
assert.equal(continuedPosts[1].content.includes("**Reward outcome**"), true);

const missingAmbientPosts = [];
const missingAmbientResult = await processDeathmarchEvents({
  events: [sensitiveEvent],
  anonymity: 3,
  dryRun: false,
  noState: true,
  ...
  fetchImpl: async (url, options) => {
    assert.equal(String(url).includes("ambient"), false);
    missingAmbientPosts.push(JSON.parse(options.body));
    return { ok: true, status: 204, text: async () => "" };
  },
});
assert.equal(missingAmbientResult.ok, true);
assert.equal(missingAmbientResult.failed, 0);
assert.equal(missingAmbientResult.posted, 1);
assert.equal(missingAmbientPosts.length, 1);
assert.equal(missingAmbientPosts[0].content.includes("**Task requested**"), true);
assert.equal(missingAmbientPosts[0].content.includes("autocorrelation"), false);
assert.equal(missingAmbientPosts[0].content.includes("ACME"), false);
