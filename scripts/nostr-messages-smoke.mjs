...
const shell = await readFile(new URL("../src/app/App.jsx", import.meta.url), "utf8");
const chatSurface = await readFile(new URL("../src/features/chat/ChatSurface.jsx", import.meta.url), "utf8");
const extensions = await readFile(new URL("../src/extensions/builtins.jsx", import.meta.url), "utf8");
assert.match(extensions, /id:\s*"messages"/);
assert.match(extensions, /label:\s*"Messages"/);
assert.match(extensions, /group:\s*"collaboration"/);
assert.match(extensions, /messagesEnabled/, "Messages should be gated by the runtime collaboration capability");
assert.match(shell, /appExtensionRegistry\.menu\("more",\s*"collaboration"/);
assert.match(shell, /navigateToView\(extension\.id\)/);
assert.match(shell, /<ExtensionSurface/, "registered More-menu views should render through the extension surface");
assert.match(chatSurface, /<ComposerSendButton/, "chat should use the shared composer send control");
