...
// The app's DATABASE_URL host (pgbouncer.<cluster>.flympg.net) names the MPG
// cluster, so the proxy always follows the database production is attached to.
function mpgClusterId() {
  if (process.env.TASKNODE_FLY_MPG_CLUSTER_ID) return process.env.TASKNODE_FLY_MPG_CLUSTER_ID;
  const [prefix, cluster] = new URL(remoteDatabaseUrl()).hostname.split(".");
  if (prefix !== "pgbouncer" || !cluster) throw new Error("Could not derive the MPG cluster from DATABASE_URL.");
  return cluster;
}

function proxyDatabaseUrl() {
...
function writeEnvFile() {
  const dbUrl = proxyDatabaseUrl();
  const body = [
    "# Generated by npm run fly-dev:data:env. This file is gitignored.",
    "# Operator runbooks source it to reach the production database through fly mpg proxy.",
