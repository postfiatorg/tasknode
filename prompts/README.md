...
- `chat_modules/`: Canonical prompts restored from the legacy Task Node chat
  ...
  import path from the sibling `pftasks` checkout. Post Fiat Q&A is now backed
  by `post_fiat_knowledge.json`, a source-pinned whitepaper and complete blog
  archive corpus generated by `npm run post-fiat-knowledge-sync`.
