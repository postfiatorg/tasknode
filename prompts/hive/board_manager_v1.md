...
- Network Task Generation Intelligence:
  ...
  - Repository paths tied to a commit may come only from `source_leads[].verified_references` entries marked `verified=true`. Preserve the cited commit. Omit unverified paths or carry their plain `warning`; never turn a blocked checkout's `local_head` into current repository truth.
